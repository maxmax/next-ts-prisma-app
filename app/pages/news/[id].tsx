...
// Функція генерації шляхів статичних сторінок
export async function getStaticPaths(ctx: GetStaticPathsContext) {
  ...
  try {
    // тут нас цікавлять дані всіх новин
  ...
  // Під час складання будуть попередньо відрендеровані тільки сторінки із зазначеними шляхами
  // `{ fallback: 'blocking' }` означає, що Next.js спробує
  // відрендерувати сторінку по відсутньому шляху на сервері
  return {
    paths,
    fallback: 'blocking'
  }
...
export async function getStaticProps({
  ...
  try {
    // тут нас цікавлять дані лише однієї новини
    const response = await fetch(
      `https://api.jsonbin.io/v3/b/${process.env.JSONBIN_BIN_ID}?meta=false`,
      {
        headers: {
    ...
    news = data[0]
		// Важливо!
		// якщо дані новини із зазначеним id відсутні,
		// Рендерим сторінку 404
    if (!news) {
      return {
        notFound: true
      }
