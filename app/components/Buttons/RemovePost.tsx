...
export default function RemovePostButton({
  ...
  // перевіряємо наявність користувача та його повноваження видалення поста
  if (!user || user.id !== authorId) return null

  const removePost = async () => {
    try {
      // повідомляємо серверу про необхідність видалення посту
      ...
      // виконуємо перенаправлення на сторінку блогу
      router.push('/posts')
    } catch (e: unknown) {
      console.error(e)
    }
