...
export default function LikePostButton({ post }: Props) {
  ...
  if (!user) return null
  // визначаємо, чи лайкал користувач цей пост
  const like = post.likes.find((l) => l.userId === user.id)
  const isLiked = Boolean(like)

	// якщо користувач лайкав пост, видаляємо лайк
	// якщо ні, створюємо лайк
	// обидва роути є захищеними
  const likePost = async () => {
    ...
    try {
      ...
      if (!res.ok) throw res
      // перезавантажуємо сторінку для повторного виклику `getServerSideProps`
